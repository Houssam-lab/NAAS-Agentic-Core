...
| تفكيك التعقيد | D-163 → D-172 · **D-182** · **D-237** · **D-239** · **D-240** (النقطة الساخنة + ميزانيات التعقيد + التوأمان + إغلاق CodeScene — `ADR-009` · تحرسها `check_endpoint_complexity`) · **D-252** (CodeScene X-Ray job 72: `chat_stream_ws` 669 سطرًا F(69) تردد 53 ⇒ قشرة استقبال ~60 سطرًا C(13) + `customer_chat_support/turn_lifecycle.py` [`handle_turn` C(12) · `_stream_and_wait` B(8) · `_close_turn` D(23)] عبر د-173 Stage 3 «القشرة + دورة الدور» — صفر تغيير سلوكي، حراس نصية تتغذى من مانيفست المركّب `_sources.py`، CI أخضر 100%) · **D-253** (CodeScene X-Ray: `agents/orchestrator.py` 552 سطرًا · خمس دوال B/C ⇒ قشرة استقبال + حزمة `orchestrator_support/` خمس شرائح نقية صفر تغيير سلوكي · مانيفست مركّب `_sources.py` · رُفِع تجميد `PLR0912` نصًا · CI أخضر 100%) · **D-254** (CodeScene X-Ray: `api_gateway/main.py` 586 سطرًا · ازدواج داخلي 4 · تردد 10 ⇒ **سجل توجيهٍ تصريحي** `ROUTE_REGISTRY` — 27 مسارًا يبني المعالجات آليًا صفر تغيير سلوكي · مانيفست مركّب `_sources.py` + حارس `check_gateway_routes_parity` endpoints لا bytes · CI أخضر 100%) |
