...
#: زمن الإبطاء الأساسي بالثواني — يتضاعف مع كل إخفاقٍ فوق العتبة.
_BASE_DELAY: float = 0.1

#: سقف الأُسّ **قبل** الرفع. `0.1 * 2**64` يتجاوز أي `MAX_DELAY` معقول بمراحل،
#: فالتشبّع مضمون؛ والغرض منع `2 ** worst` من أن يصير عدداً صحيحاً هائلاً يرفع
#: `OverflowError` (انظر `_dilation_delay`).
_MAX_DILATION_EXPONENT: int = 64
...
class ChronoShield:
    ...
    def __init__(self):
        ...
        # ⛔ **لا عتبة قفلٍ صلب للعنوان — عمداً** (D-236 · القاعدة 10 في العقيدة).
        #
        # كانت هنا `IP_LOCKOUT_THRESHOLD = 200`، وكانت **تناقض القانون الذي يحكمها**:
        # `AUTHENTICATION_DOCTRINE.md:62` يقول «القفل الصلب (429) على الهوية وحدها».
        # ومئتان ليست رقماً آمناً بل رقماً **مؤجِّلاً**: 200 إخفاق في نافذة ٥ دقائق من
        # عنوان carrier-NAT جزائري واحد أمرٌ عاديّ في ذروة المساء — والنتيجة أنّ كل
        # طالب خلف ذلك العنوان يتلقّى 429 **بكلمة سرّه الصحيحة**. أي أنّ ISS-152 كان
        # باقياً بعتبةٍ أكبر لا أكثر. رصدته مراجعة CodeRabbit، وكان مُحقّاً.
        #
        # المتّجه العنواني يبقى حيّاً في `_dilation_delay` وحده: يُكلِّف المهاجم ثوانيَ
        # ولا يمنع أحداً الخدمة.
    ...
    async def check_allowance(self, request: Request, identifier: str) -> None:
        ...
        الآن: `ip:` يحمل العنوان الحقيقي (`client_identity`)، و**القفل الصلب على
        الهوية وحدها** — فخطأ طالبٍ لا يمسّ جاره. العنوان يُبطِئ ولا يقفل أبداً،
        مهما بلغ عدد إخفاقاته: carrier-NAT الجزائري يجعل آلافاً يتقاسمون عنواناً
        واحداً، فأيّ عتبةِ قفلٍ عنوانية — مهما ارتفعت — هي ISS-152 مؤجَّلاً.

        ⚠️ **الحدّ المتبقّي مُعلَن:** مهاجمٌ يدوّر الهويات من عنوان واحد يواجه
        إبطاءً أُسّياً مسقوفاً بـ`MAX_DELAY` فقط. هذا **يُكلِّفه ولا يُوقفه** —
        وهو ثمنٌ مقبولٌ بوعي: القفل العنواني يمنع الخدمة عن آلاف الأبرياء لإبطاء
        مهاجمٍ واحد، والحسابُ الفردي محميٌّ بقفل الهوية على أي حال. الدفاع الحقيقي
        ضدّ حشو بيانات الاعتماد طبقةٌ أعلى (WAF/حدّ معدّل على الحافّة)، لا هنا.
        """
        # Maintenance cycle
        ...
        # ⛔ **ولا فرعَ قفلٍ صلب للعنوان هنا** — القاعدة 10: القفل على الهوية وحدها.
        # كان الفرع موجوداً بعتبة 200، فكان يعيد إنتاج ISS-152 برقمٍ أكبر (انظر
        # التعليق على الثوابت أعلاه). العنوان يُبطِئ في المرحلة 2 ولا يمنع.
    ...
    def _dilation_delay(self, *, target_fails: int, ip_fails: int) -> float:
        ...
        ⛔ **الأُسّ مسقوفٌ قبل الرفع لا بعده.** `min(0.1 * 2**worst, MAX_DELAY)`
        تبدو مسقوفة وهي ليست كذلك: القوس الداخلي يُحسَب أوّلاً، و`2 ** 4980` عددٌ
        صحيح هائل يرفع `OverflowError` عند التحويل إلى float — أي **500 على كل
        محاولة دخول من ذلك العنوان**، بمن فيهم الأبرياء. كان الفرع غير قابلٍ
        للبلوغ ما دام القفل العنواني يسبقه؛ ولمّا حُذف القفل (القاعدة 10) صار
        بلوغه مسألة وقتٍ لا احتمال. كشفه الاختبار الذي كُتب لحراسة الحذف نفسه —
        وهو الفرق بين حذفٍ محروس وحذفٍ متفائل.
        """
        overages = [
        ...
        if worst <= 0:
            return 0.0
        # ما بعد نقطة التشبّع لا يغيّر النتيجة: `MAX_DELAY` يبتلع كل زيادة.
        return min(_BASE_DELAY * (2 ** min(worst, _MAX_DILATION_EXPONENT)), self.MAX_DELAY)
