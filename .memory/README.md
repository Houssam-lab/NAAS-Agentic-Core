...
| `decisions.md` | سجلّ القرارات المعمارية D-001→**D-258** (ADR log — **D-258: تفكيك hotspot بنية الاختبارات `tests/conftest.py` (416 سطرًا · CodeScene X-Ray 2026-08-15):** قشرة معمارية + حزمة شرائح `tests/conftest_support/` نقية (helpers · registry بعزل حقيقي + قفل المحرك ISS-113 · schema يفكّ `_reset_db_steps` — Bumpy Road مغلق · lifecycle مراحل · auth_shards · policy · مانيفست `_sources.py`) — اكتشاف معماري: pytest لا يفعّل autouse لfixtures مستوردة — قشور التسجيل بقيت في الconftest · صفر تغيير سلوكي · الحزمة الكاملة مطابقة · **D-255: قشرة + شرائح `content_support/`** لتفكيك hotspot `tools/content.py` (CodeScene churn 40): قشرة استقبال A(1) + ست شرائح نقية · مانيفست `_sources.py` · ContentSearchArgs يقتل PLR0917 · صفر تغيير سلوكي · ruff + pytest + E2E أخضر؛ **D-254: سجل التوجيه التصريحي** `ROUTE_REGISTRY` لبوابة الـ API — 27 مسارًا يبني المعالجات آليًا صفر تغيير سلوكي · ratchet «endpoints لا bytes») | سجلّ ملزِم |
| `issues.md` | سجلّ الكوارث المُشخَّصة والمُصلَحة ISS-001→**ISS-170** (**ISS-158 (الحسابات والرسائل تختفي: سقوطٌ صامت إلى SQLite في الذاكرة — ✅ أغلق في D-246 + D-247: رفض إقلاعٍ حقيقي (exit 1 صريح؛ الإقلاع كان يموت فعليًا في 1602daf — exit 1 دفاعٌ استباقي ضد استدعاء if/\|\|) + فحصٌ حيّ للمنافذ + سقف الرسائل 20→100 مع cursor has_more/next_before_id — ترقيم الواجهة دَين (لاحظ: ALLOW_EPHEMERAL_DB ومحلّل urlparse ادّعاهما توثيقٌ فثبت بـgrep أنهما لم يُدخلا في الكود — وثّق فقط ما يُختبر))** · **ISS-159 (كلّ سؤالٍ يُعامَل كتمرين احتمالات: تصنيفُ التوجيه يعرف ثلاثة مواضيع رياضية فقط — مفتوح)** · **ISS-160 (`--update` عفوٌ صامت في بوّابة التعقيد — ✅ أغلق في D-244: خريطة التعقيد تُحدَّث بقرارٍ مُرقَّم لا صمتًا)**؛ **ISS-153 (راية `persisted` تُقتَل قبل السلك ⇒ كتابة مزدوجة على ذراع التراجع — مفتوح، D-238)** · **ISS-154 (دورٌ فارغ صامت + تسريب نصّ استثناء — مفتوح)** · **ISS-155 (توأما WS — ✅ أُغلق في D-239: نسخةٌ واحدة لمنطق الدور)**؛ **ISS-156 (ترميزٌ مزدوج ⇒ تسمّم `customer_messages`) وISS-157 (النموذج المحظور بـD-067 افتراضاً حيّاً) أُغلقا في D-238**؛ **ISS-163 ✅ أغلق في D-245 + D-246** — كارثة المنصة الأربعة في وجهٍ واحد: نشر Replit static بلا Backend (login = 404/مرفوض على الرابط المنشور) + `/health` أخضر كاذب حتى وDB ميت + إقلاع لا يصرخ عند حجب Supabase (6543/5432) فتسقط المصادقة والحفظ والإجابات بصمت + D-112 (لا إجابة إلا من orchestrator-service) — عولجت بمجسّات حيّة صادقة معلَنة (D-245); **ISS-161 ✅ أغلق في D-241** — «Login failed» يعود بعد كل Codespace جديدة: مفتاح التوقيع من القرص المتقلب ⇒ K-ROOT (طبقة `app_state` في Supabase · لا ضبط كلمة مرور إلا بـ`ADMIN_FORCE_PASSWORD_SYNC=1` · E2E 24/24)؛ **ISS-162 أغلق في D-244** — بوابتان كاذبتان: خريطة التعقيد المجمّدة تتقادم بصمتٍ بعد كل تفكيكٍ بنيوي مشروع فتكذب عن الواقع، واختبارا create_mission أخضران بلا رمز JWT يغطّيان الثغرة نفسها التي يسدّها D-242 — الخريطة تُحدَّث بقرارٍ مُرقَّم لا صمتًا، والاختبار الأخضر بلا معنى مقياسٌ مُفسَّر لا كذّاب; **ISS-152 أُغلق في D-236** — «Login failed»: عقدُ أخطاءٍ مُعلَن بنصفه فوق قفلٍ عالمي بعنوان مشترك؛ **ISS-151 (أيقونات من نطاقٍ ثالث — مفتوح مُخفَّف، D-232)** · ISS-150 (تسريب شظايا لاتينية — مفتوح)؛ (ISS-149 أُغلق في D-208؛ ISS-148 في D-207؛ ISS-144 في D-206؛ **ISS-145 أُغلق بالتفنيد في D-209** — الـ212 صفّاً تحمل `ui_component` كلّها، و`truly_silent = 0` على كامل الإنتاج؛ يبقى ISS-137 · **ISS-141 (تدوير مفاتيح — إجراء المالك)** · **ISS-164 ✅ أغلق في D-252** — كارثة الـ hotspot المدمّر: `chat_stream_ws` 669 سطرًا · تعقيد F(69) · تردد تغيير 53 ⇒ قشرة استقبال ~60 سطرًا C(13) + دورة دور مفككة `customer_chat_support/turn_lifecycle.py` (handle_turn C12 · _stream_and_wait B8 · _close_turn D23 — ratchet منطوق per-file-ignores) عبر D-173 Stage 3 «القشرة + دورة الدور» صفر تغيير سلوكي · حراس نصية تتغذى من مانيفست المركّب `_sources.py` · CI أخضر 100% · ISS-142 · **ISS-146 (توجيه تربوي بدور user)** · **ISS-147 (تذبذب 401)** مفتوحة)؛ **ISS-166 ✅ أغلق في D-254** · **D-257 ✅ أغلق في ISS-169** — سقوط صامت للمحادثات (مستوى مستخدم فقط) عند بدء جلسة DB أولى: `User.missions` في `app/core/domain/user.py` يشير إلى `Mission` عبر string forward-ref مع `foreign_keys` كنص ⇒ `InvalidRequestError` غير معلَن عند أول `save_message` لمسار customer chat (E2E: id=4915 نجح بعدها) ⇒ استيراد `Mission` حرفيًا + `foreign_keys=[Mission.initiator_id]` مرجع كائني صفر تغيير سلوكي في غير إصلاح المسار · E2E حي على Supabase: login ×2 · WS 317 حدثًا · health=ok · CI أخضر 100%. **ISS-168 ✅ أغلق في D-256** — كارثة الـ hotspot الخامسة: `orchestrator_client.py` 238 سطرًا · Code Duplication على `get_mission`/`get_mission_events` (15/16 سطرًا) · `_has_indexed_match` أعلى تردد (38 سطرًا · churn=2 · embedded import) ⇒ قشرة تفويض حرفية + حزمة شرائح `orchestrator_client_support/` (`missions.py`: `_request_mission` قلب موحد + `ServiceJwtPayload` بيانات معلنة للـ JWT · `preempts.py`: `resolve_indexed_anchor` يعزل قرار Supabase) · مانيفست مركّب `_sources.py` يتغذى منه حارسا legacy_invariants وskills_doctrine الموسّعان · 11 اختبارًا سلوكًا جديدًا + القديمة 13/13 أخضر · ruff 0.14.0 أخضر · **ISS-167 ✅ أغلق في D-255** — كارثة الـ hotspot الرابعة (CodeScene job 72 · churn 40): `tools/content.py` 173 سطرًا · `search_content` C(14) · 112 سطرًا · 9 وسائط ⇒ قشرة استقبال A(1) + حزمة شرائح `content_support/` (ست شرائح نقية: normalize_branch · scan_for_error بمراحل نقية — Bumpy Road مغلق · SearchFilters · build_search_context · build_search_report · مانيفست `_sources.py`) · ContentSearchArgs dataclass (نمط D-249) يقتل PLR0917 مع الحفاظ على التوقيع العام · قشور late-binding للأسماء القديمة · ruff 0.14.0 أخضر · pytest 10/10 · E2E ToolRegistry أخضر · runtime truth + القفل محدّثان · ISS-166 ✅ أغلق في D-254** — كارثة الـ hotspot الثالثة: `api_gateway/main.py` 586 سطرًا · ازدواج داخلي 4 · تردد تغيير 10 ⇒ **سجل توجيهٍ تصريحي** `ROUTE_REGISTRY` (27 مسارًا) يبني المعالجات آليًا صفر تغيير سلوكي · مانيفست مركّب `_sources.py` · حارس `check_gateway_routes_parity` · **ISS-165 ✅ أغلق في D-253** — كارثة الـ hotspot الثانية: `agents/orchestrator.py` 552 سطرًا · خمس دوال B/C تعقيد متجاوز الحد (مركز ثقل لمحة البحث والشرح والاحتياطية والتاريخ والاسترجاع) ⇒ قشرة استقبال تفوض + حزمة شرائح دورة الدور `orchestrator_support/` (خمس شرائح نقية) صفر تغيير سلوكي · مانيفست مركّب `_sources.py` · رُفِع تجميد `PLR0912` نصًا (ratchet يتقلص) · runtime truth + القفل محدّثان · pytest 14/14 · CI أخضر 100%. | سجلّ ملزِم |
