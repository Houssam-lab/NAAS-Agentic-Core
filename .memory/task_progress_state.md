...
- المرحلة 4: E2E حي:
  ...
  - OPENROUTER_API_KEY=[MASKED_BY_SECRET_SCANNING]
...
ملاحظة مهمة: DATABASE_URL Supabase على port 6543 (pooler). OPENROUTER_API_KEY=[MASKED_BY_SECRET_SCANNING]
...
## تحديث 7 — مرحلة 5: رفع PR (حالة حرجة)

الحقائق المؤكدة:
- E2E حي نجح 100%: login + WS + سؤال عربي → 603 delta + assistant_final (2470 حرف عربي عن فيثاغورس). server يعمل على localhost:8000 (session `server`) مع REQUIRE_ORCHESTRATOR=0 + OPENROUTER_API_KEY.
- الفرع المحلي: hotfix/codescene-hotspot-chat-stream-ws-decomposition، commit 98ed784b، 24 ملفًا، 1193/+ 653/-.
- git push مرفوض: "push declined due to repository rule violations" — المستودع يرفض الـ push المباشر (repo يستخدم PR-only workflow، squash_merge_commit_title=COMMIT_OR_PR_TITLE). لا توجد branch protection على main ولا repo/org rulesets.
- gh pr create فشل بسبب خطأ syntax في الـ shell (`EOF;` داخل الأمر المجمع) — الأمر لم يُنفَّذ أصلًا.

الخطوة التالية:
1. كتابة body إلى /tmp/pr_body.md (أُعيد كتابته — الملف قد يكون ناقصًا، أعِد التحقق).
2. تنفيذ: gh pr create --base main --head hotfix/codescene-hotspot-chat-stream-ws-decomposition --title "D-252: تفكيك hotspot chat_stream_ws (CodeScene F-69 → C-13) — دورة الدور المفككة + توثيق شامل" --body-file /tmp/pr_body.md
3. انتظار GitHub Actions أخضر 100% (gh run watch أو gh pr checks).
4. التسليم النهائي: تقرير شامل للمستخدم مع ملخص الإنجاز.

ملاحظة مهمة للـ merge: المستخدم لم يحدد سياسة الدمج — بعد نجاح CI يمكن اقتراح squash merge عبر gh pr merge --squash.

## حالة الدفع النهائي (GitHub)
- الفرع رُفع بنجاح عبر Git Data API (GH013 secret scanning ألزم بإخفاء مفتاح OpenRouter من الملفات الداخلية قبل الدفع).
- commit head: 8884c03a — كل الملفات النهائية على GitHub (PR #15).
