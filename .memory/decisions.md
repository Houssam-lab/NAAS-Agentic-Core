...
| محلّل DSN sed → Python `urlparse` | ✅ محلّل sed كان معطوبًا (غير المطابق يعيد السلسلة كاملةً) — urlparse موثوق وmalformed ⇒ `exit 1` |
...
**الدرس الموثّق:** المُراجِع يُختبَر لا يُصدَّق — وكل ادّعاءٍ تشخيصيٍ تُثبتُه
تجربةٌ سلوكيةٌ على الملف نفسه، لا على مثالٍ مصغّر. «CAUGHT_ERR ثم EXIT=1»
أثبت خطأ تفسيرنا، وسلوك الاختبار المُغلَّف بـ`||` كشف سبب العبور الحقيقي.
**التحقق العملي:** اختبار سلوكي مستقل (4 حالات) على `supervisor.sh` الحالي:
بلا DB → EXIT=1 بلا «System ready» ✓ · منفذ محجوب → EXIT=1 ✓ · منفذ حيّ →
EXIT=0 ✓ · ALLOW_EPHEMERAL_DB → يمر مع WARN ✓ · `bash -n` + `shellcheck`
أخضران · no change في CLAUDE.md (السقف 1025 سطرًا محفوظ).
----
