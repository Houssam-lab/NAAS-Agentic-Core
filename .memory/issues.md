...
دائماً فتح حساب جديد، ولا أجد رسائلي المحفوظة.»

⛔ **لم يُعدَّل سطرٌ واحد** — رُصد أثناء تشخيص ISS-155 وسُجِّل بقرار المالك (لا مساس
بـ`supervisor.sh` ولا `APP_DATABASE_URL` ولا طبقة قاعدة البيانات في ذلك الـPR).

**الخيط الأوّل — السقوط الصامت:** `.devcontainer/supervisor.sh:200-219` حين يغيب
`DATABASE_URL` الحقيقي من بيئة العملية يكتب `sqlite+aiosqlite:///:memory:` **ويُقلع
التطبيق على أيّ حال**، ويُبقي `ENVIRONMENT=development` (تعليقه يستشهد بـD-095/ISS-094).
فيُرجِع `/health` قيمة `ok` بينما كلّ حسابٍ ومحادثةٍ ورسالة تعيش في ذاكرة العملية —
**وكلّ إقلاع يمسح كلّ شيء**. ويُفاقمه `:507-525` («D-SQLITE-GUARD»): يقرأ
`/proc/$pid/environ` ويُجبر إعادة تشغيل uvicorn كلّما رأى `sqlite` — أي أنّ الوضع
المتدهور يمسح نفسه مرّةً أخرى بالتصميم. هذا خرقٌ مباشر لـ§0 «لا فشل صامت».

طبقة التطبيق **سليمة**: `app/core/settings/helpers.py:109-122` ترفع في `development`
ولا تفترض شيئاً؛ التدهور يحدث في المُقلِع وحده.

**الخيط الثاني — لماذا لا يرى الطالب ما حدث:**
`app/services/boundaries/auth_boundary_service.py:302-305` يُرجِع **404** لرمزٍ صحيح
التوقيع يشير إلى صفٍّ غير موجود، بينما `frontend/app/components/CogniForgeApp.jsx:548`
تُسجّل الخروج على **401/403 فقط** وتعامل ما عداهما «خللاً عابراً»؛ فيبقى الملفّ الشخصي
المخبَّأ في `localStorage` معروضاً ويظنّ الطالب أنه داخل. ونفس الحالة تُرجِع **401** في
`app/api/routers/customer_chat.py:113-117` — رمزان لحالةٍ واحدة (نفس صنف ISS-152).

**الخيط الثالث — سقفٌ صلب على التاريخ:**
`app/services/boundaries/customer_chat_boundary_service.py:307,348` يقرأ `limit=20`
بينما افتراض طبقة الاستمرارية 1000 (`chat_persistence.py:193-196`)؛ فآخِر **٢٠** رسالة
فقط تعود من أيّ محادثة، بلا مسار ترقيم صفحات. مساهمٌ مستقلّ في «لا أجد رسائلي».

**ملكية الصفوف سليمة** (`chat_persistence.py:168-191` يُرشِّح `user_id`) — فليست تسرّباً.

**شرط الإغلاق:** (١) لا إقلاع على قاعدةٍ زائلة بلا **رفضٍ صريح** أو وسمٍ يبلغ الواجهة؛
(٢) رمزٌ واحد لحالة «المستخدم غير موجود» في الطرفين مع سلوك خروجٍ متّسق (يمدّد
`check_error_contract_parity` من D-236)؛ (٣) ترقيم صفحات للتاريخ أو سقفٌ مُصرَّح يراه
المستخدم.

---

## ISS-159 (2026-08-12) — كلّ سؤالٍ يُعامَل كتمرين احتمالات: تصنيفُ التوجيه يعرف ثلاثة مواضيع رياضية فقط — 🔴 مفتوح
