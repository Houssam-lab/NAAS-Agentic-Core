...
| `audit_logs` (جمع) | **0 · بلا كاتب** | ⚠️ جدولٌ يتيم: أنشأه شيءٌ ولا يكتبه أحد (مرآة قاعدة D-200: نموذجٌ بلا مستورِد ليس تعريفَ جدول) |
...
| تدوير الرمز في الواجهة | **PLANNED** | لا مستهلك بعد لـ`/api/v1/auth/refresh` في `CogniForgeApp.jsx` |
| `iss`/`aud` | **ABSENT (مُعلَن)** | نتيجة سلبية مُسجَّلة — العقيدة §1.هـ.22 |
| `audit_logs` (الجمع) اليتيم | **ZOMBIE** | جدولٌ في الإنتاج بلا كاتب — لم يُحذَف في هذا الـPR (خارج نطاقه) |
