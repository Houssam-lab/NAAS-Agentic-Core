...
def current_changed_paths() -> list[str]:
    ...
    paths: set[str] = set()
    if base:
        paths.update(line.strip() for line in run_git(["diff", "--name-only", "--diff-filter=ACMRTUXB", f"{base}...HEAD"]).splitlines() if line.strip())
    else:
        for args in (
...
def fingerprint(paths: list[str]) -> str:
    ...
    for relative in paths:
        if relative == PACKET_REL:
            continue
        path = ROOT / relative
        files = [path] if path.is_file() else sorted(child for child in path.rglob("*") if child.is_file()) if path.is_dir() else []
...
def main() -> int:
    ...
    if any(payload is None for payload in (packet, matrix, evidence, curriculum, offers)):
        return 1
    assert packet is not None and matrix is not None and evidence is not None and curriculum is not None and offers is not None
    ...
    snapshot_paths = snapshot.get("changed_paths_excluding_packet", []) if isinstance(snapshot, dict) else []
    if not isinstance(snapshot, dict) or not snapshot.get("content_sha256_excluding_packet"):
        ...
        fail(f"packet changed-path snapshot is stale: recorded={len(snapshot_paths)}, actual={len(actual_paths_excluding_packet)}")
    elif snapshot.get("content_sha256_excluding_packet") != fingerprint(actual_paths_excluding_packet):
    ...
    if not isinstance(standards, list) or not standards:
        ...
        for identifier in standards:
            ...
            if row.get("status") in {"PENDING_CLASSIFICATION", "EXTERNAL_ABSENT"}:
                fail(f"packet cannot use non-active source as authority: {identifier} ({row.get('status')})")
            if row.get("runtime_allowed") is not False and row.get("status") == "MANDATORY_REFERENCE":
    ...
    catalog_course_ids = {str(row.get("course_id")) for row in curriculum_rows if isinstance(row, dict)}
    ...
    considered_ids = {str(identifier) for identifier in curriculum_packet.get("considered_course_ids", [])}
    if considered_ids != catalog_course_ids:
        fail(f"curriculum coverage mismatch: considered-only={sorted(considered_ids - catalog_course_ids)}, catalog-only={sorted(catalog_course_ids - considered_ids)}")
    ...
    for path in (curriculum_packet.get("catalog_path"), curriculum_packet.get("application_matrix_path")):
    ...
    for field in ("owner", "status", "interfaces", "failure_behavior_ar", "observability_ar", "security_boundary_ar", "rollback_ar"):
    ...
    if FAILURES:
        ...
        return 1
    passed("Code acceptance packet is complete: standards, full curriculum consideration, evidence, local application, production proof, commercial trace, and zero deletions are present.")
    return 0
